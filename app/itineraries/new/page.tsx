...
import { ENERGIE_TO_FUEL, estimateFuelCost } from '@/lib/fuel-price'
...
export default function NewItineraryPage() {
  ...
  const [fuelPrice, setFuelPrice] = useState<FuelPriceResult | null>(null)
    ...
    <PageTransition>
      <div className="min-h-screen bg-ivory pt-[52px] flex">
        ...
        <div className="w-[380px] flex-shrink-0 border-r border-sand/60 p-10 overflow-y-auto">
          <Link
            href="/itineraries"
            className="font-inter text-[8px] tracking-[0.15em] uppercase text-stone hover:text-deep-blue transition-colors duration-150 flex items-center gap-2 mb-8"
          >
            ← Mes itinéraires
          </Link>
        ...
        <div className="flex-1 overflow-y-auto p-10">
          <div className="flex items-baseline justify-between mb-2">
            <p className="font-inter text-[8px] tracking-[0.2em] uppercase text-stone">
              Aperçu — {preview.totalVillages} villages · {preview.totalDistanceKm} km
            </p>
          ...
          {vehicle && fuelPrice && preview.totalDistanceKm > 0 && vehicle.conso_mixte !== null && (
            <div className="flex items-center gap-3 mb-3">
              <p className="font-inter text-[8px] tracking-[0.12em] uppercase text-stone">
                Carburant estimé
              </p>
              <p className="font-cormorant italic text-[20px] text-deep-blue leading-none">
                {estimateFuelCost(preview.totalDistanceKm, vehicle.conso_mixte, fuelPrice.price).toFixed(2)} €
              </p>
              <p className="font-inter text-[7px] tracking-[0.1em] text-stone/50">
                ({fuelPrice.price.toFixed(3)} €/L · {vehicle.conso_mixte} L/100km)
              </p>
            </div>
          )}
